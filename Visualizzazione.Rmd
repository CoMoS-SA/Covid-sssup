...
epidemia_regioni %>% 
  ...
  geom_dl(aes(label = regione_evidenza), method = "last.points", cex = 0.5) +
...
dati_regionali %>%
  ...
  scale_color_discrete("Ripartizione") + 
  theme(legend.position = "top") +
  labs(
    title = "Totale cumulato casi COVID-19 per regione o provincia autonoma",
    subtitle = paste0("Ordinate per numero di casi al ", format(max(dati_regionali$data), "%d %B %Y")),
...
dati_regionali %>%
  ...
  arrange(data, rango_regione) %>% 
  mutate(regione = fct_inorder(regione)) %>% 
  ggplot(aes(x = data, y = nuovi_deceduti, fill = ripartizione_5)) +
  geom_col() +
  scale_y_log10() + annotation_logticks(sides = "l") +
  scale_x_date(date_labels = "%d %b") +
  facet_wrap(~regione, ncol = 5) +
...
dati_regionali %>% 
  ...
  labs(
    ...
    x = NULL, y = NULL,
    subtitle = "Regioni maggior numero di casi positivi"
  )
```
...
dati_regionali %>% 
  ...
  theme(legend.position = "top") +
  labs(
    title = "Ricoveri ospedalieri e terapia intensiva",
    subtitle = "Incremento giornaliero",
    x = NULL, y = "Numero ricoveri"
  )
```  
